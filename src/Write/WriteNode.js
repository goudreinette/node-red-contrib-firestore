...
class FirestoreWriteNode {
  ...
  onInput(msg, send, errorCb) {
    ...
    switch (op) {
      ...
      case 'update':
      case 'delete':
        this.firestore.collection(col).doc(doc)[op](JSON.parse(JSON.stringify(payload)))
            .then(handleSuccess)
            .catch(handleFailure)
        break;
      default:
